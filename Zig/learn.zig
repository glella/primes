...
// Random Numbers
// Here we create a new prng using a 64 bit random seed. a, b, c, and d are given random values via this prng. 
// The expressions giving c and d values are equivalent. DefaultPrng is Xoroshiro128; there are other prngs available 
// in std.rand.
...
// Threads
// While Zig provides more advanced ways of writing concurrent and parallel code, std.Thread is available for making 
// use of OS threads. Let’s make use of an OS thread.
...
test "threading" {
    ...
    _ = thread;
    try expect(tick == 0);
    std.time.sleep(3 * std.time.ns_per_s / 2); // std.time.sleep(3 * std.time.ns_per_s / 2);
    try expect(tick == 1);
}
...
// Hash Maps
// The standard library provides std.AutoHashMap, which lets you easily create a hash map type from a key type and 
...
// std.StringHashMap and std.AutoHashMap are just wrappers for std.HashMap. If these two do not fulfil your needs, 
// using std.HashMap directly gives you much more control.

// If having your elements backed by an array is wanted behaviour, try std.ArrayHashMap and its wrapper 
// std.AutoArrayHashMap.

// Stacks
// std.ArrayList provides the methods necessary to use it as a stack. Here’s an example of creating a list of matched 
// brackets.
...
// std.sort.asc and .desc create a comparison function for the given type at comptime; if non-numerical types should 
// be sorted, the user must provide their own comparison function.

// std.sort.sort has a best case of O(n), and an average and worst case of O(n*log(n)).

// Iterators
// It is a common idiom to have a struct type with a next function with an optional in its return type, so that the 
// function may return a null to indicate that iteration is finished.
...
// Some iterators have a !?T return type, as opposed to ?T. !?T requires that we unpack the error union before the 
// optional, meaning that the work done to get to the next iteration may error. Here is an example of doing this with 
// a loop. cwd has to be opened with iterate permissions in order for the directory iterator to work.

test "iterator looping" {
    var iter = (try std.fs.cwd().openIterableDir(".", .{})).iterate(); // fixed
...
// Here we will implement a custom iterator. This will iterate over a slice of strings, yielding the strings which 
...
// std.fmt.fmtSliceHexLower and std.fmt.fmtSliceHexUpper provide hex formatting for strings as well as {x} and {X} 
// for ints.
...
// std.fmt.fmtIntSizeDec and std.fmt.fmtIntSizeBin output memory sizes in metric (1000) and power-of-two (1024) 
// based notation.
...
// Advanced Formatting
// So far we have only covered formatting specifiers. Format strings actually follow this format, where between 
// each pair of square brackets is a parameter you have to replace with something.
...
// Cross compilation
// By default, Zig will compile for your combination of CPU and OS. This can be overridden by -target. 
// Let’s compile our tiny hello world to a 64 bit arm linux platform.
