; primes.s
; compiles with: cl65 -t cx16 -o CHECKER.PRG -l checker.list checkprimes.s 
...
number:     .word 0             ; div vars
divisor:    .word 0
remain:     .word 0
divresult:  .word 0

num:        .word 0             ; root variables
rem:        .word 0
temp:       .word 0
root:       .byte 0

isPrime:    .byte 0             ; main vars 
count:      .word 0
limit:      .word 0
ivar:       .word 0
;nvar:       .word 0             ; not needed just use divisor
;nvar = divisor                  ; reuse same register as it does not change during division & avoids assignments
printnum:   .word 0
scratch:    .word 0
...
; prompts
prompt:             .asciiz "enter test number: "
num_error_prompt:   .asciiz "must be a number:  "
prime_prompt:       .asciiz "is it a prime?     "
root_prompt:        .asciiz "root + 1:          "
remainder_prompt:   .asciiz "remainder:         "
repeat_num:         .asciiz "num:               "
repeat_number:      .asciiz "number:            "
...
start:
    PRINT_STRING prompt
    jsr get_number
    MoveW input_binary, num
    MoveW input_binary, number
    ; lda input_binary
    ; sta num
    ; sta number
    ; lda input_binary+1
    ...
    ; sta number+1
    jsr checkPrime
    PRINT_STRING prime_prompt
    lda remain
    ;cmp #$00
    beq @notaprime
    PRINT_STRING yes_string
    lda #NEWLINE
    jsr CHROUT
    rts
@notaprime:
    PRINT_STRING no_string
    lda #NEWLINE
    jsr CHROUT
    rts
...
@loopcheck:
    ...
    MoveW input_binary, number  ; reset number to be divided
    jsr divide              ; trial division 
    lda remain              ; if remainder is zero it is not a prime
	beq @exit	            ; branch if equal (zero flag 1 after compare)
