; primes.s
; compiles with: cl65 -t cx16 -o PRIMES.PRG -l primes.list italiano.s
...
@next:
    ...
    lda ivar+1
    sta number+1    ; reset number high for division

    inc root        ; int(sqr(i))+1
    lda root
    sta Kval        ; --> K
    lda #1
    sta Jval
LJ:
    inc Jval
    ldx Jval
    stx divisor     ; save division vars
    lda ivar
    sta number
    lda ivar+1
    sta number+1
...
@skip:
    ...
    lda remain      ; check if the remainder is zero
    beq @exit       ; if zero exit

    ldx Jval
    cpx Kval
    bne LJ          ; if Jval != Kval next j
...
@cont2:
    ...
    bne outerloopdone       ; ishigher - ivar is higher than limit
...
continue:
    ;jsr checkPrime         ; do the actual checking
    lda #1                  ; set nvar / divisor / isPrime to 1 (same register)
	sta nvar	
    sta isPrime
    lda ivar                ; load num (root) & number (division) with ivar
    sta num
    sta number
    lda ivar+1
    sta num+1
...
loopcheck:
	...
	inc nvar
	lda nvar                ; have we finished checking?
	cmp root		        ; test divide until sqrt of number
    ;bcc @check              ; islower
    ;beq @check              ; issame
    ;bne outerloop           ; ishigher
    bcs outerloop
@check:
    jsr divide
    lda remain
    beq finishedcheck       ; not a prime continue looping
    jmp loopcheck
finishedcheck:
    lda remain


processPrime:               ; if it is a prime inc prime counter and print it
    inc count
    bne @done_inc
    inc count+1 
@done_inc:
...
; 	sta isPrime				
; 	sta nvar		
;     lda ivar            ; load num (root) & number (divide) with ivar
;     sta num
;     sta number
;     lda ivar+1
;     sta num+1
